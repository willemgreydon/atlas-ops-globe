...
export async function GET() {
  ...
  // sets the feed status. GDACS (floods/cyclones/droughts) and FIRMS (dense
  // active fires over Africa/Siberia/China/Australia) are SUPPLEMENTARY: they
  // only add data when healthy and never drag the feed to mock/cached if they
  // hiccup (mergeArrayResults takes the worst status). FIRMS needs FIRMS_MAP_KEY.
  const [eonet, usgs, gdacs, firms] = await Promise.all([
  ...
  const data = [...merged.data, ...gdacsData, ...firmsData];
  const source = ["eonet+usgs", gdacsData.length ? "gdacs" : "", firmsData.length ? "firms" : ""].filter(Boolean).join("+");
