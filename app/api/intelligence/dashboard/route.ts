...
import { projectCooccurrence } from "@/lib/intel/graph";
...
function build(): unknown {
  ...
  // Entity relationship graph — project the bipartite (entity–hub) relationships
  // onto entity↔entity co-occurrence so persons/orgs/countries connect directly
  // (see lib/intel/graph.ts). The top 200 by co-occurrence strength, edges seen
  // in ≥ 2 shared hubs.
  const { nodes: graphNodes, edges: graphEdges } = projectCooccurrence(
    rels.map((r) => ({ a: str(r.a), b: str(r.b) })),
    ent,
    { maxNodes: 200, minWeight: 2 },
  );
